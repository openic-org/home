...
<p style="text-align:center"><i><b>Figure 2.</b> System architecutre of the Headstage.</i></p>

<br>

![](../images/headstage-schematic.png)

<p style="text-align:center"><i><b>Figure 3.</b> Schematic of the Headstage.</i></p>
...
|    R64    |        RHD2164        |    Intan     | Recording chip    |
|    S16    |        RHS2116        |    Intan     | Stimulation chip  |
...
<p style="text-align:center"><i><b>Figure 4.</b> Rendering of the Headstage PCB (left) top and (right) bottom sides.</i></p>
...
<p style="text-align:center"><i><b>Figure 5.</b> Details of the electrode array connector footprint.</i></p>

<br>

We describe each pin type below:

<p style="text-align:center"><i><b>Table 2.</b> Electrode array connector pins.</i></p>
...
<p style="text-align:center"><i><b>Figure 8.</b> Daisy-chain configuration of `ADGS5414` octal analog switches.</i></p>

The `GND` for the analog switches is shared with the `R64` and `S16` chips.


## R64 & S16

The `R64` and `S16` *Intan* chips are used here without any modification from their usual configuration. Both `R64` have a shared supply `VDD1` coming from the recording controller, and are set up with `LVDS` outputs. The `S16` chip has independent supplies of `VDD2`, `VSTIMp`, and `VSTIMm` coming from the stimulation controller, and is set up with `LVDS` outputs. All 3x chips share the same `REF` electrode and the same `GND`. The `GND` is also shared with the analog switches.
...
<p style="text-align:center"><i><b>Table 3.</b> Interface connector pin assignment.</i></p>
...
<p style="text-align:center"><i><b>Figure 9.</b> Details of the Interface Connector footprint.</i></p>
...
We use an 8 layer stack where we prioritize shielding of the traces connecting the electrode array with the analog switches and recording chip `R64`. The table below shows a summary of our strategy. The top L1 and bottom L2 layers are used for minimal local routing around the components and `GND` plane. Layer L2 is used for supply and SPI routing of the analog switches. Adding a `GND` plane allows us to also use this layer for sensitive routing between the electrode pins and the analog switches (see figure below). Layer L3 is used as a `GND` plane to shield any signal routing from the switches supplies and SPI routing. Layers L4, L5, and L6 are used for analog routing between the electrode pins, switches, recording `R64`, and stimulation `S16` chips. Layer L4 is also used to route the `R64` supplies from the interface connector. This routing is localized to the far right side of the board and shielded with `GND` planes in layers L3, L4, and L5. Layer L7 is used for some additional local routing between the switches and to bring the SPI signals to the interface connector. The SPI routing is shielded with a `GND` plane in layers L6, L7, and, where possible, in layer L8.
...
<p style="text-align:center"><i><b>Table 4.</b> Headstage PCB specifications.</i></p>
...
<p style="text-align:center"><i><b>Table 5.</b> Headstage PCB Stack Up.</i></p>
...
<p style="text-align:center"><i><b>Figure 10.</b> PCB layout with detailed routing for each layer.</i></p>
...
<p style="text-align:center"><i><b>Figure 11.</b> Reference electrode routing.</i></p>
...
<p style="text-align:center"><i><b>Table 6.</b> Headstage PCB Vias.</i></p>

| Layer  |     Via1      |  &#956;-via1  |  &#956;-via2  |  &#956;-via3  |  &#956;-via4  |  &#956;-via5  |  &#956;-via6  |  &#956;-via7  |
| :----: | :-----------: | :-----------: | :-----------: | :-----------: | :-----------: | :-----------: | :-----------: | :-----------: |
|   1    | $\textemdash$ | $\textemdash$ |               |               |               |               |               |               |
|        |  $\textbar$   |  $\textbar$   |               |               |               |               |               |               |
|   2    |  $\textbar$   | $\textemdash$ | $\textemdash$ |               |               |               |               |               |
|        |  $\textbar$   |               |  $\textbar$   |               |               |               |               |               |
|   3    |  $\textbar$   |               | $\textemdash$ | $\textemdash$ |               |               |               |               |
|        |  $\textbar$   |               |               |  $\textbar$   |               |               |               |               |
